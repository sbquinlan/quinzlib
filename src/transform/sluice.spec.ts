...
describe('sluice', () => {
  it('should work with empty iterator', async () => {
    const result = await sink(fluent(range(0), sluice(3, 100)));
    expect(result).toEqual([]);
  });

  it('should work with 1', async () => {
    const start = Date.now();
    const result = await sink(fluent(range(10), sluice(1, 100)));
  ...
  it('should work with 3', async () => {
    const start = Date.now();
    const result = await sink(fluent(range(10), sluice(3, 100)));
    expect(result).toEqual([...range(10)]);
