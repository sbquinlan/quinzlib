import { backoff, fluent, range, sink, map, sluice } from '.';
...
describe('backoff', () => {
  ...
  it('should validate base', async () => {
    const thrower = new ThrowTill(2);
    const result = backoff(() => thrower.try(), -20.4, 3);
    await expect(result()).resolves.toEqual(2);
  });
